...
This module exists to prove that `halcytone.contracts` v0.3.0 is actually
consumable from a real downstream caller. If the contracts subpackage adds,
...
declares which reserved streams it needs to see from its publishers. Later
sprints will move this to a richer roster-negotiation protocol; for now
it's a flat list fed to `validate_stream_roster`.
"""
...
def expected_streams() -> tuple[str, ...]:
    """Return the stream roster `halcytone.core` v0.3.0 needs from publishers.

    Minimal scope: one channel of each modality that the fusion layer
    will eventually consume. Wider rosters (full 4-channel EEG, both HRV
    variants, IMU) land in v0.4.0 once the fusion implementation lands.
    """
    return (
        "eeg.ch1",
        "ppg",
