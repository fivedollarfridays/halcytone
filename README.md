...
Biofeedback sonification platform. Monorepo with subpackages under `halcytone.*` — shared wire-protocol schemas, session state, and (in later releases) sensor adapters, audio synthesis, live HUD, and session publishing.

Shipped as **v0.3.0** of the unified package. See [`ROADMAP.md`](ROADMAP.md) for scope bands and [`CHANGELOG.md`](CHANGELOG.md) for what has changed.

## Why this exists

Halcytone is a personal biofeedback sonification platform. Multiple sensors publish live signal streams, a core fusion engine aligns and combines them, and downstream modules turn the combined state into audio, visuals, and recorded sessions. v0.3.0 consolidates what used to be the separate `halcytone-contracts` and `halcytone-core` repos into a single monorepo, so the shared contract surface and its primary consumer ship together.
...
| `halcytone.core` | Fusion, session lifecycle, storage orchestration | Stub (v0.3.0); real fusion in v0.4.0 |
...
Shipped subpackages re-export their full public surface through the top-level `halcytone` namespace, so callers can write `from halcytone import SignalPacket` and treat submodule layout as an implementation detail.
...
check_contract_version("0.3.0")  # the version this consumer was developed against
...
`halcytone.core` consumes `SignalPacket`s and emits a fused `StateVector`. Every downstream module reads `StateVector`s, not raw signals.
...
**SQLite** at `~/halcytone/halcytone.db`: session metadata, baselines, annotations, computed session summaries. Queryable for longitudinal analysis. Tables: `sessions`, `baselines`, `annotations`, `state_summaries`. Full DDL in `halcytone.contracts.storage`.
...
Manifest schema (shortened; full schema lives in `halcytone.contracts.bundles` and the generated `manifest.schema.json`). Since v0.2.0, `baselines` and `summary` are typed pydantic models (`Baseline`, `SessionSummary`) rather than free-form dicts:
...
`Baseline.streams` keys are validated against `RESERVED_STREAMS` — unknown names are rejected at parse time. `summary_schema_version` lets consumers branch on shape as the summary evolves in later releases.
...
Policy (see `halcytone/contracts/drift.py` for the table):
...
The JSON Schema mirror of `SessionManifest` at `halcytone/contracts/bundles/manifest.schema.json` is checked into the repo. CI regenerates it via `scripts/regen_manifest_schema.py` and fails on any diff, so the TypeScript side (AgentGrounds) can trust the file as the canonical cross-language contract.
